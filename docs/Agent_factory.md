# hexDAG Agent Factory: Architectural Design and Multi-Tier Framework
...
hexDAG Agent Factory addresses the fundamental challenges in AI orchestration with a **three-tier architecture**.
...
2. **Agent Factory** → declarative workflows, registries, macros, and policies
...
├─────────────────────────────────────────────────────────────┤
│ Tier 2: Agent Factory (hexai.agent_factory)                  │
...
## 🏗️ Tier 2: Agent Factory

### Design Philosophy: Systematic Composition & Customization

Agent Factory transforms the Core into an AI orchestration platform, balancing **low-code workflow design** with **deep extensibility** through registries.
...
* Official: auto-verified at build-time from `hexai.core.application.nodes`
...
While Core and Agent Factory remain cloud-agnostic, enterprises need **native integration** with their platforms. Tier 3 provides that via modular plug-ins.
...
* **Tier 2 (Agent Factory)** → Customizable orchestration with registries, YAML, macros, policies, and multi-orchestrator routing
