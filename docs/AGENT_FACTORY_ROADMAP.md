...
### 1.1 Core Registry System (`hexai.core.registry`)

* **NodeTypeRegistry:** Framework node factories with build-time verification
* **Build script:** Auto-generate official nodes list from `hexai.core.application.nodes`
...
### 1.2 Agent Factory Registry System (`hexai.agent_factory.registry`)
...
metadata:
  ...
  namespace: my-org
spec:
  type: CustomConsensusNode  # From agent_factory custom registry
---
apiVersion: hexai/v1
kind: Pipeline
spec:
...
## Phase 3: Custom Extension System in Agent Factory

**Objective:** Agent Factory as extension platform for custom components

### 3.1 Custom Node Framework (`agent_factory/custom/`)
...
### 3.2 Custom Pipeline Patterns (`agent_factory/patterns/`)
...
### 3.3 Custom Function Library (`agent_factory/functions/`)
...
### 4.1 Standard Registry (`agent_factory/registry/standard.py`)
...
### 4.2 MLflow-Style Registry (`agent_factory/registry/versioned.py`)
...
### 5.1 Macro Architecture (`agent_factory/macros/`)
...
### 6.1 Policy Framework (`agent_factory/policies/`)
...
### 6.2 Multi-Orchestrator Architecture (`agent_factory/orchestration/`)
...
### 7.2 Event Sink (`agent_factory/events/`)
