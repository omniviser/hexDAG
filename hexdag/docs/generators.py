...
def _strip_line_trailing_ws(text: str) -> str:
    """Strip trailing whitespace from each line."""
    return "\n".join(line.rstrip() for line in text.splitlines())


# Path to the generated schema
SCHEMA_PATH = Path(__file__).parent.parent.parent / "schemas" / "pipeline-schema.json"
...
"""
    ...
    def generate_cli_reference(self) -> str:
        """Generate CLI reference by running ``hexdag --help`` and subcommand help.

        Uses subprocess to capture actual CLI output from Typer, so the
        reference always matches the installed version.

        Returns
        -------
        str
            Complete CLI reference as markdown
        """
        lines = [
        ...
        # Discover subcommands from CLI command modules
        import pkgutil

        from hexdag.cli import commands as cmd_pkg

        subcommands: list[tuple[str, str]] = []
        for _finder, mod_name, _is_pkg in pkgutil.iter_modules(cmd_pkg.__path__):
            if mod_name.startswith("_"):
                continue
            try:
                mod = __import__(
                    f"hexdag.cli.commands.{mod_name}",
                    fromlist=["_CLI_NAME", "_CLI_HELP"],
                )
                name = getattr(mod, "_CLI_NAME", None)
                help_text = getattr(mod, "_CLI_HELP", "")
                if name:
                    subcommands.append((name, help_text))
            except Exception:
                continue
        ...
        for cmd_name, cmd_help in subcommands:
            lines.append(f"### `hexdag {cmd_name}`")
            lines.append("")
            if cmd_help:
            ...
            try:
                result = subprocess.run(
                    ["uv", "run", "hexdag", cmd_name, "--help"],
                    capture_output=True,
                    text=True,
                    timeout=30,
                )
                cmd_output = _strip_line_trailing_ws(result.stdout.strip() or result.stderr.strip())
                if cmd_output:
                    lines.append("```")
    ...
    def generate_yaml_reference(self) -> str:
        """Generate YAML manifest reference from schema and compiler plugins.

        Documents all manifest kinds (Pipeline, Macro, Config) with their
        structure and fields.

        Returns
        -------
        str
            Complete YAML manifest reference as markdown
        """
        lines = [
            ...
            (
                "This reference is auto-generated from the pipeline JSON schema "
                "and compiler plugin definitions."
            ),
            "",
            "## Manifest Kinds",
            "",
            ...
            "",
            (
                "Multiple kinds can appear in a single file using YAML "
                "multi-document syntax (`---` separator)."
            ),
            "",
        ]
        ...
        lines.extend([
            ...
            "",
            (
                "Define reusable node templates with parameters. Macros are "
                "expanded inline at build time."
            ),
            "",
            "### Structure",
            "",
