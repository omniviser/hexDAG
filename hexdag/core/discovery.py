...
1. Builtin components from hexdag.builtin.*
...
Examples
--------
>>> from hexdag.core.discovery import discover_plugins, discover_modules
>>> plugins = discover_plugins()
>>> modules = discover_modules("hexdag.builtin.adapters")
...
def discover_modules(package_path: str) -> list[str]:
    ...
    ----------
    package_path : str
        Dotted path to the package (e.g., "hexdag.builtin.adapters")
...
def discover_user_modules() -> list[str]:
    ...
    """
    try:
        from hexdag.core.config.loader import load_config
    except ImportError:
        return []
