...
from hexdag.cli.commands.registry_cmd import ComponentType, bootstrap_registry, registry
...
def run_pipeline(
    ...
    for node in sorted_nodes:
        ...
        # Ensure we have a callable before invoking
        func_callable = func if callable(func) else None

        if not func_callable:
            console.print(f"[red]Node function '{func_name}' not callable[/red]")
            raise typer.Exit(1)
