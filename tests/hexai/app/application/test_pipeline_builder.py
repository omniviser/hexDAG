...
from hexai.app.application.pipeline_builder import PipelineBuilder, PipelineBuilderError
...
"""
        ...
        with pytest.raises(PipelineBuilderError, match="Invalid field_mapping_mode"):
            self.builder.build_from_yaml_string(yaml_content)
...
"""
        ...
        with pytest.raises(PipelineBuilderError, match="custom_field_mappings required"):
            self.builder.build_from_yaml_string(yaml_content)
...
"""
        ...
        with pytest.raises(
            PipelineBuilderError, match="input_mapping for node 'node1' must be a dictionary"
        ):
            self.builder.build_from_yaml_string(yaml_content)
