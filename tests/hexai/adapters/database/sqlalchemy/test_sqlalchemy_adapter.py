...
from hexai.adapters.database.sqlalchemy.sqlalchemy_adapter import SQLAlchemyAdapter
...
async def adapter(mock_table):
    ...
    with patch(
        "hexai.adapters.database.sqlalchemy.sqlalchemy_adapter.create_async_engine",
        return_value=engine,
    ):
