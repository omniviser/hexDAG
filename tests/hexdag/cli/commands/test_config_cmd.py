"""Tests for hexdag.cli.commands.config_cmd module."""
...
from hexdag.cli.commands.config_cmd import (
    _display_dict,
    app,
...
def mock_registry():
    ...
    with patch("hexdag.cli.commands.config_cmd.registry") as mock:
        yield mock
...
def mock_console():
    ...
    with patch("hexdag.cli.commands.config_cmd.console") as mock:
        yield mock
...
class TestEnsureRegistryReady:
    ...
    def test_bootstrap_when_not_ready(self, mock_registry):
        ...
        with patch("hexdag.cli.commands.config_cmd.bootstrap_registry") as mock_bootstrap:
            ensure_registry_ready()
...
class TestGenerate:
    ...
    def test_generate_with_plugins(self, runner, mock_registry, tmp_path):
        ...
        with patch("hexdag.cli.commands.config_cmd.registry.bootstrap"):
            with runner.isolated_filesystem(temp_dir=tmp_path):
                result = runner.invoke(app, ["generate", "--plugin", "hexdag.adapters.test"])
...
"""
            ...
            with patch("hexdag.cli.commands.config_cmd.load_config") as mock_load:
                mock_config = MagicMock()
                mock_config.modules = ["hexdag.core.ports"]
                mock_config.plugins = []
...
class TestShow:
    ...
    def test_show_default_config(self, runner):
        ...
        with patch("hexdag.cli.commands.config_cmd.load_config") as mock_load:
            mock_config = MagicMock()
            mock_config.modules = ["hexdag.core.ports"]
            mock_config.plugins = ["hexdag.adapters.test"]
            mock_config.dev_mode = True
