...
class TestOrchestrator:
    ...
    async def test_ports_with_mocks(self, orchestrator, observers):
        ...
        # Create ports with ToolRouter and a simple mock function
        mock_llm = MockLLM(responses=["Test LLM response"])
        tool_router = ToolRouter(
            tools={
                "test_tool": lambda **kw: {"status": "success", "tool": "test_tool"},
            }
        )
        ports = {"llm": mock_llm, "tool_router": tool_router, "observers": observers}
